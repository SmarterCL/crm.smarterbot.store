    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error(t('notConfigured', { setup: t('setupTab') }));
        } else {
          toast.error(data.error ?? t('getReplyFailed'));
        }
      ...
      toast.error(t('reachAgentFailed'));
      setTurns(turns);
      setInput(text);
    } finally {
      setSending(false);
    ...
    <div className="flex h-[60vh] min-h-[420px] flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">{t('playgroundHeader')}</span>
